...
RUN apt-get install -y mercurial build-essential libpcre3 libpcre3-dev zlib1g zlib1g-dev curl git cmake ninja-build golang gnutls-bin iptables
RUN git clone --depth=1 https://github.com/google/boringssl.git --single-branch -b master
...
RUN cmake -GNinja ..
RUN ninja
RUN mkdir -p /boringssl/.openssl/lib && \
    cp /boringssl/build/crypto/libcrypto.a /boringssl/build/ssl/libssl.a /boringssl/.openssl/lib && \
    ln -s /usr/include /boringssl/.openssl && \
    mkdir -p /boringssl/.openssl/include/openssl && \
